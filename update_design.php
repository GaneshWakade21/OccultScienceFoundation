...
<body>
  <div class="main-page-container">
    ...
    <div class="floating-cushion">
      <div class="incoushin">
        <div class="heading">Update Student Details</div>
        ...
        <form action="#" method="post">
          ...
          <div>
            <input type="text" name="first-name" value="<?php echo $row['first_name']; ?>" placeholder="First Name"
              class="courier">
            <input type="text" name="middle-name" value="<?php echo $row['middle_name']; ?>" placeholder="Middle Name"
              class="courier">
            <input type="text" name="last-name" value="<?php echo $row['last_name']; ?>" placeholder="Last Name"
              class="courier">
            <input type="text" name="fathers-name" value="<?php echo $row['fathers_name']; ?>"
              placeholder="Fathers Name" class="courier">
          </div>
          <div>
            <input type="text" name="phone-number" value="<?php echo $row['mob_no']; ?>" placeholder="Phone-number"
              class="courier">
            <input type="text" name="email" value="<?php echo $row['email_id']; ?>" placeholder="Email" class="courier">
            <input type="text" name="address" value="<?php echo $row['address']; ?>" placeholder="Address"
              class="courier">
            <input type="text" name="pin_code" value="<?php echo $row['pin_code']; ?>" placeholder="PinCode"
              class="courier">
          </div>
          <div class="form-row">
            <select class="visitor" name="coursename" placeholder="Select Course Name">
              <option value="0">Select Course Name</option>
              <option value="tarot" <?php
              if ($row['course_name'] == 'tarot') {
                echo "selected";
              }
              ?>>Tarot Cards Course
              </option>
              <option value="chaledean" <?php
              if ($row['course_name'] == 'chaledean') {
                echo "selected";
              }
              ?>>Chaledean
                Numerology Course</option>
              <option value="lalkitaab" <?php
              if ($row['course_name'] == 'lalkitaab') {
                echo "selected";
              }
              ?>>Lal Kitaab
                Course</option>
              <option value="vastu" <?php
              if ($row['course_name'] == 'vastu') {
                echo "selected";
              }
              ?>>Vastu Shastra Course
              </option>
              <option value="vedic" <?php
              if ($row['course_name'] == 'vedic') {
                echo "selected";
              }
              ?>>Vedic Astrology Course
              </option>
              <option value="mobilecourse" <?php
              if ($row['course_name'] == 'mobilecourse') {
                echo "selected";
              }
              ?>>Mobile
                Numerology Course</option>
            ...
            <select name="coursetype" placeholder="Select Course Name" class="details">
              ...
              if ($row['course_type'] == 'envelope') {
                echo "selected";
              }
              ?>>Basic</option>
              <option value="box" <?php
              if ($row['course_type'] == 'box') {
                echo "selected";
              }
              ?>>Advance</option>
            </select>
          ...
          <div class="fee">
            <input type="text" value="<?php echo $row['fee']; ?>" placeholder="total fees" name="fee">
          </div>
          <div class="form-row">
            <select name="pay_type" placeholder="Select Payment Method">
              <option value="Payment Type">Payment Type</option>
              <option value="fullpayment" <?php
              if ($row['pay_type'] == 'fullpayment') {
                echo "selected";
              }
              ?>>Full Payment
              </option>
              <option value="emi" <?php
              if ($row['pay_type'] == 'emi') {
                echo "selected";
              }
              ?>>EMI</option>
            ...
            <select name="upi" placeholder="Select Payment Method">
              ...
              <option value="upi" <?php
              if ($row['upi'] == 'upi') {
                echo "selected";
              }
              ?>>UPI</option>
              <option value="gpay" <?php
              if ($row['upi'] == 'gpay') {
                echo "selected";
              }
              ?>>GPay</option>
              <option value="phonepay" <?php
              if ($row['upi'] == 'phonepay') {
                echo "selected";
              }
              ?>>PhonePay</option>
              <option value="paytm" <?php
              if ($row['upi'] == 'paytm') {
                echo "selected";
              }
              ?>>Paytm</option>
          ...
          <div>
            <input type="text" value="<?php echo $row['upi_no']; ?>" name="UPINumber" placeholder="UPI Number"
              class="courier">
            <input type="text" value="<?php echo $row['amount_paid']; ?>" name="AmountPaid" placeholder="Amount Paid"
              class="courier">
          </div>
          <div class="form-row">
            <span class="details">Transction Date</span>
            <input value="<?php echo $row['trans_date']; ?>" value="<?php echo $row['trans_date']; ?>" type="date"
              name="trans_date" required>
          </div>
          <div class="form-row">
            <span class="details">Transction Time</span>
            <input value="<?php echo $row['trans_time']; ?>" type="time" name="trans_time" required>
          </div>
          <div class="form-row">
...
if ($_POST['register']) {
  ...
  $query = "UPDATE coursebooking set id='$id' , first_name = '$first_name',middle_name ='$middle_name',last_name='$last_name',
fathers_name='$father_name',mob_no = '$mob_no',email_id='$email_id', address='$address',pin_code='$pincode',course_name='$course_name',
course_type='$course_type',batch = '$batch',fee='$fee',pay_type='$pay_type', upi='$upi', upi_no='$upi_no' ,amount_paid='$amount_paid',
trans_date='$trans_date' ,trans_time='$trans_time',trans_slip='$trans_slip',certificate='$cer',remark='$remark'";
  $data = mysqli_query($mysqli, $query);
  if($data){
 echo "<script>alert('Recorde Update Sucefully');</script>";
}
else{
